...
class MemberModel extends Model 
{
	...
	public function _before_update(&$data, $option)
	{
            ...
            return $this->error='come tu before_update';
	...
	public function getPrivilege()
	{
        ...
        public function addFaceimg(){
            if(isset($_FILES['face']) && $_FILES['face']['error']==0){
                ...
                $info = $upload->uploadOne($_FILES['face']);  //上传文件后,返回值中包含了--图片路径和名称.拼接起来就可以使用他制作缩略图


                $data['face'] = $info['savepath'].$info['savename'];
                $data['sm_face'] = $info['savepath']."sm132_".$info['savename'];


     //拼接好缩略图路径--交给对应的字段--在controller中一起给model--add插入
                $image = new \Think\Image();
                //先把缩略图对应值--给数据表设置好---->再压缩图片
                $image->open(IMG_URL_HD . $data['face']);

                $image->thumb(132,132)->save(IMG_URL_HD . $data['sm_face']);
